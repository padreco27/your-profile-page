    ...
    <section ref={ref} id="portfolio" className="py-24 md:py-32 bg-secondary/40">
      <div className="container">
        <div className="text-center max-w-xl mx-auto mb-16">
          <h2 className={`font-heading text-3xl md:text-4xl font-bold tracking-tight text-balance ${visible ? "animate-fade-up" : "opacity-0"}`}>Nossos projetos</h2>
          <p className={`mt-4 text-muted-foreground text-lg ${visible ? "animate-fade-up" : "opacity-0"}`} style={{ animationDelay: "100ms" }}>
            Veja alguns dos sites que já criamos para nossos clientes.
          </p>
        </div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {projects.map((project, i) => (
            <div key={project.title} className={`group relative overflow-hidden rounded-xl border border-border ${visible ? "animate-fade-up" : "opacity-0"}`} style={{ animationDelay: `${150 + i * 100}ms` }}>
