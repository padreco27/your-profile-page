    ...
    <section ref={ref} id="servicos" className="py-24 md:py-32">
      <div className="container">
        <div className="text-center max-w-xl mx-auto mb-16">
          <h2 className={`font-heading text-3xl md:text-4xl font-bold tracking-tight text-balance transition-all duration-700 ${visible ? "animate-fade-up" : "opacity-0"}`}>O que fazemos</h2>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {services.map((service, i) => (
            <div key={service.title} className={`group rounded-xl border border-border bg-card p-8 transition-all duration-300 hover:border-primary/40 hover:shadow-lg hover:shadow-primary/5 ${visible ? "animate-fade-up" : "opacity-0"}`} style={{ animationDelay: `${150 + i * 80}ms` }}>
